...
var Api = (function (ignoreHeaders) {
  ...
  Api.prototype.setBasicAuthorization = function (username, password) {
    var user_pwd = new Buffer(username + ":" + password).toString('base64');
  ...
  Api.prototype.getIgnoreHeadersLowerCase = function () {
    if (!this.ignoreHeadersLowerCase) {
  ...
  Api.prototype.useRavenHq = function (apiKey, cb) {
    return request.get({
      ...
      return request.get({
        ...
        return cb && cb(err, authorizationHeaderValue);
      });
    });
  };
...
module.exports = Api(IGNORE_HEADERS);
