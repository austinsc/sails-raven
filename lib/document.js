...
var Document = function Document(values, schema, collection) {
  ...
  var idprefix = collection.identity + '/';

  if (values) {
    // Find the primary key and update the raven key
    var pkey = linq.from(Object.keys(schema)).singleOrDefault(function (x) {
      return x.primaryKey;
    });
    if (pkey && values[pkey]) {
      if (values[pkey].indexOf('\\') || values[pkey].length > 1023) {
        ...
        self.setMetadataValue('Key', idprefix + values[pkey]);
      }
    } else {
      self.setMetadataValue('Key', idprefix);
    }
    ...
    linq.from(Object.keys(values)).where(function (x) {
      return _.has(self.schema, x) && values[x] !== null && values[x] !== undefined;
