 ...
 * @api public
 */
Collection.prototype.save = function save(values, cb) {
  var self = this;
  ...
  var docs = values.map(function (value) {
    return new Document(value, self.schema, this).values;
  });

  var bulkdocs = docs.map(function (doc) {
    return {
      Method: 'PUT',
      Key: doc.getMetadataValue('key'),
      Document: doc,
      Metadata: doc.getMetadata()
    };
  });

  this.connection.db.bulkSave(docs, function (err, results) {
    cb(err, results);
  });
};
