  ...
  try {
    ...
    if (where.id) {
      db.getDocument(this.identity + '/' + where.id, function (err, result) {
        cb(err, new Document(result, self.schema, self));
      });
    } else {
      db.find(where, function (err, results) {
        if (err) {
          ...
          async.map(results, function (result, done) {
            var doc = new Document(result, self.schema, self);
            peek4(doc);
            done(null, doc);
          }, cb);
        } else {
          var doc = new Document(results, self.schema, self);
          peek3(doc);
          cb(null, doc);
        }
      });
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  self.find(criteria, function (err, result_s) {
    ...
    async.each(results, function (result, done) {
      self.connection.db.deleteDocument(result['@metadata']['@id'], done);
    }, function (err) {
      cb(err, results);
    });
  });
...
Collection.prototype._createDocument = function () {
