...
Collection.prototype.find = function find(criteria, cb) {
  ...
  if (query.aggregate) {
    ...
    return db.aggregate(aggregate, function (err, results) {
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  self.connection.db.find(query.criteria.where, function(err, results){
    async.each(results, function(result, done){
        peek3(result['@metadata']['@id']);
