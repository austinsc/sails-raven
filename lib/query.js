...
Query.prototype.parseExpression = function parseExpression(field, expression) {
  ...
  if (_.isArray(expression)) {
    return {
      $in: self.parseValue(field, '$in', expression)
    };
  }
...
Query.prototype.parseValue = function parseValue(field, modifier, val) {
  ...
  if (hop(self.schema, field)) {
    if(self.schema[field].primaryKey && self['@metadata'] && self['@metadata']['@id']) {
    ...
    if (_.isString(val)) {
      ...
      // as strings.
      if (self.schema[field].type != 'string') {
        if (val === "false") {
          return false;
        }
