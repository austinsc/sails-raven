...
Database.prototype.queryIndex = function(filter, cb) {
  ...
  if (typeof filter === 'string') {
    ...
    // console.log('Query Filter  :  ' + look(filter));
    // this.getDocument(filter, function(err, doc) {
    //   cb(err, [doc]);
    // });
    ...
    // console.log('Query Filter  :  ' + look(filter.id || filter.where.id));
    // this.getDocument(filter.id || filter.where.id, function(err, doc) {
    //   cb(err, [doc]);
    // });
  } //else {
